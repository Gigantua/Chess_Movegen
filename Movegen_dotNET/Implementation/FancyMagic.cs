...
{
    public static class FancyMagic
    {
        static ulong[] lookup_Table = new ulong[88507];
        ...
        static FancyMagic()
        {
            for (int x = 0; x < 8; x++)
            {
                ...
                {
                    int sq = y * 8 + x;
                    ulong rMask = RelevantRookSee(x, y);
                    ulong bMask = RelevantBishopSee(x, y);

                    int cnt = BitOperations.PopCount((ulong)rMask);
                    for (ulong i = 0; i < (ulong)(1 << cnt); i++)
                    {
                        ulong occ = Bmi2.X64.ParallelBitDeposit(i, rMask);
                        lookup_Table[rIndex(sq, occ)] = Switch.Rook(sq, occ);
                    }

                    cnt = BitOperations.PopCount((ulong)bMask);
                    for (ulong i = 0; i < (ulong)(1 << cnt); i++)
                    {
                        ulong occ = Bmi2.X64.ParallelBitDeposit(i, bMask);
                        lookup_Table[bIndex(sq, occ)] = Switch.Bishop(sq, occ);
                    }
                }
            }
        }
        ...
        static int rIndex(int s, ulong occ)
        {
            FancyHash m = r_magics[s];
            return m.offset + (int)(((occ | m.mask) * m.hash) >> (64 - 12));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        static int bIndex(int s, ulong occ)
        {
            FancyHash m = b_magics[s];
            return m.offset + (int)(((occ | m.mask) * m.hash) >> (64 - 9));
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ulong Rook(int s, ulong occ)
        {
            return lookup_Table[rIndex(s, occ)];
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static ulong Bishop(int s, ulong occ)
        {
            return lookup_Table[bIndex(s, occ)];
        }
