...
namespace Chess_Lookup::SlideArithm
{
	...
	/* Start of code */
	static const inline uint64_t slide_arithmetic(uint32_t sq, uint64_t block) {
		//BZHI
		//[src & (1 << inx) - 1] ;
		// split the line into upper and lower rays
		uint64_t mask = bzhi(block, sq);
	...
	static const inline uint64_t Queen(uint32_t sq, uint64_t occ)
	{
		const uint64_t* r = rank_mask.data() + 4 * sq;
		return slide_arithmetic(sq, r[0] & occ) & r[0]
			 ^ slide_arithmetic(sq, r[1] & occ) & r[1]
			 ^ slide_arithmetic(sq, r[2] & occ) & r[2]
			 ^ slide_arithmetic(sq, r[3] & occ) & r[3];
	}
}